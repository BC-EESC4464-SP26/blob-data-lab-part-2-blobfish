...
ooiLon = -144.3979;
% -->
latDif = abs(ooiLat - woa.lat);
[minLatDif, indlat] = min(latDif);

lonDif = abs(ooiLon - woa.lon);
[minLonDif, indlon] = min(lonDif);
...
% -->
% 1 decibar of pressure is basically 1 meter of depth in the ocean
ooiPressure = 29.4607;
depthDif = abs(ooiPressure - woa.depth);
[minDepthDif, inddepth] = min(depthDif);
...
Interp_Woa = interp1(woa_time, woa_papa_rep, full_times);
...
%ncdisp(filename);
satLat = ncread(filename, "latitude");
satLon = ncread(filename, "longitude");
satTime = ncread(filename, "time");
satTemp = ncread(filename, "sstAnom");

time_days = satTime / 86400;
time0 = datenum("1970-01-01 00:00:00");
satellite_time_final = time0 + time_days;
Datestring = datestr(satellite_time_final);
...
satLatDif = abs(ooiLat - satLat);
[minSatLatDif, satIndLat] = min(satLatDif);

satLonDif = abs(ooiLon - satLon);
[minSatLonDif, satIndLon] = min(satLonDif);
...
sat_sstanom = squeeze(satTemp(satIndLon,satIndLat,:));
Interp_sat = interp1(satellite_time_final, sat_sstanom, full_times);
plot(full_times, Interp_sat, ".")
datetick("x", 22);
hold off;